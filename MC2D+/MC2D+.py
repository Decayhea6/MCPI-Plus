#Import JPG Images into walls in minecraft
...
path = ''
def findImg():
    root.directory = filedialog.askopenfilename(filetypes=[('JPEG', '*.jpg' )])
    global path
    path = root.directory
    print('Location is: ' + path)

def importImg():
    mc.postToChat('Importing Image')
    selfie_rgb = io.imread(path)
...
Title = Label(root, text = 'MC2D')
Subtitle = Label(root, text = 'Import .jpg Images into Minecraft')
findFile = Button(root, text = 'Import', command = findImg)
impFile = Button(root, text = 'Generate', command = importImg)
